...
Vue 3 (Composition API, `<script setup>`) · TypeScript ~6.0 · Vite 8 · Pinia · Vue Router 4 · Tailwind CSS 3 (dual palette, see Styling) · Supabase (auth + Postgres/PostGIS + RLS + real-time) · Leaflet · Midtrans Snap (client-side simulator, see Architecture notes) · Axios · Lucide Vue Next · `@vueuse/core`
...
- **No real payment backend**: this project is a pure Vite SPA + Supabase, with no server component that could safely hold a Midtrans server key. Midtrans Snap is intentionally kept as a **client-side simulated modal** — real data (anak/langganan/pembayaran) is persisted to Supabase around it, but "payment confirmation" is a client action (`konfirmasiPembayaranLunas`) rather than a real Midtrans webhook. Don't try to add a real server-side Midtrans flow without discussing the hosting/backend implication first.
...
- **Third-party integrations**: Midtrans Snap (simulated, see above), WhatsApp via Fonnte (`whatsappLayanan.ts`), Supabase Realtime (`realtimeLayanan.ts` — live GPS/status subscriptions, e.g. `pantauSupirRealtime`), Google Maps/Waze deep links (`navigasiLayanan.ts`), and a payment reconciliation flow (`rekonsiliasiLayanan.ts`) tying Midtrans callbacks back to `pembayaran`/`penundaan_pembayaran`.
